...
addon:RegisterEvent("RAISED_AS_GHOUL")
addon:RegisterEvent("COMBAT_LOG_EVENT_UNFILTERED")

addon:SetScript("OnEvent", function(self, event, ...)
	if addon.events[event] then
		addon.events[event](self, event, ...)
	end
end)

-- A simple timer function to replace C_Timer.After
...
timerFrame:SetScript("OnUpdate", function(self, elapsed)
    for i = #timerQueue, 1, -1 do
        ...
        if timer.delay <= 0 then
            timer.func()
            tremove(timerQueue, i)
        end
    end
end)
function addon:After(delay, func)
    tinsert(timerQueue, {delay = delay, func = func})
end

function addon:RegisterEvent(event, func)
	addon.events[event] = func
end
...
function OpenRecap(recapID)
	...
	if not deathEvents or #deathEvents <= 0 then
		for i = 1, 5 do self.DeathRecapEntry[i]:Hide() end
	...
	for i = #deathEvents + 1, #self.DeathRecapEntry do self.DeathRecapEntry[i]:Hide() end
...
function Amount_OnEnter(self)
	...
	if self.spellName then
		if self.caster then
			GameTooltip:AddLine(format("%s by %s", self.spellName, self.caster), 1, 1, 1, true)
		else
	...
	if seconds > 0 then
		GameTooltip:AddLine(format("%s sec before death at %s%% health.", format("%.1F", seconds), self.hpPercent), 1, 0.824, 0, 1)
	else
		GameTooltip:AddLine(format("Killing blow at %s%% health.", self.hpPercent), 1, 0.824, 0, true)
	end
	GameTooltip:Show()
end
...
function CreateDeathRecapFrame()
	...
	DeathRecapFrame = CreateFrame("Frame", "StandaloneDeathRecapFrame", UIParent)
	...
	DeathRecapFrame.Title = DeathRecapFrame:CreateFontString(nil, "ARTWORK", "GameFontNormalLarge")
	DeathRecapFrame.Title:SetPoint("TOP", 0, -9)
	DeathRecapFrame.Title:SetText("Death Recap")

	DeathRecapFrame.Unavailable = DeathRecapFrame:CreateFontString(nil, "ARTWORK", "GameFontNormal")
	DeathRecapFrame.Unavailable:SetPoint("CENTER")
	DeathRecapFrame.Unavailable:SetText("Death Recap unavailable.")

	local closeXButton = CreateFrame("Button", "$parentCloseButton", DeathRecapFrame, "UIPanelCloseButton")
	closeXButton:SetPoint("TOPRIGHT", 8, 8)

	DeathRecapFrame:SetScript("OnDragStart", DeathRecapFrame.StartMoving)
	DeathRecapFrame:SetScript("OnDragStop", DeathRecapFrame.StopMovingOrSizing)
	...
	for i = 1, 5 do
		...
		button.DamageInfo.Amount = button.DamageInfo:CreateFontString(nil, "ARTWORK", "GameFontNormalRight")
		...
		button.DamageInfo.AmountLarge = button.DamageInfo:CreateFontString(nil, "ARTWORK", "NumberFont_Outline_Large")
		...
		button.SpellInfo.Name = button.SpellInfo:CreateFontString(nil, "ARTWORK", "GameFontNormal")
		...
		button.SpellInfo.Caster = button.SpellInfo:CreateFontString(nil, "ARTWORK", "SystemFont_Shadow_Small")
	...
	StaticPopupDialogs["KDEATH_STANDALONE"] = {
		...
		OnCancel = function(self, data, reason)
			if reason == "override" or reason == "timeout" then return end
		...
		OnAlt = function(self)
			addon:After(0.01, function()
				if not StaticPopup_FindVisible("KDEATH_STANDALONE") then
					StaticPopup_Show("KDEATH_STANDALONE", GetReleaseTimeRemaining(), SECONDS)
				end
			end)
...
addon:RegisterEvent("PLAYER_LOGIN", CreateDeathRecapFrame)

local function HideDeathPopup()
	StaticPopup_Hide("KDEATH_STANDALONE")
end

addon:RegisterEvent("PLAYER_ENTERING_WORLD", HideDeathPopup)
addon:RegisterEvent("RESURRECT_REQUEST", HideDeathPopup)
addon:RegisterEvent("PLAYER_ALIVE", HideDeathPopup)
addon:RegisterEvent("RAISED_AS_GHOUL", HideDeathPopup)
...
addon:RegisterEvent("COMBAT_LOG_EVENT_UNFILTERED", function(_, _, timestamp, event, _, srcName, srcFlags, dstGUID, dstName, dstFlags, ...)
	if (bit_band(dstFlags, COMBATLOG_FILTER_ME) ~= COMBATLOG_FILTER_ME) or (bit_band(srcFlags, COMBATLOG_FILTER_ME) == COMBATLOG_FILTER_ME) or (not validEvents[event]) then
		return
	end
