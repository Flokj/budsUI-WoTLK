...
if C.Misc.PvPTimer ~= true then return end

-- PvP start timer (retail-style): autostart countdown bar + big center
-- numbers on BG/arena start, invite countdown on the battlefield popup.
-- Ported from FriskesUI-WoTLK CollectionAddons/RetailTimer.
-- Test macro: /script RunFakeTestTimer:OnEvent("START_TIMER", 1, 15, 60)

local ADDON = K.Directory
local PATH_SOUND = "Interface\\AddOns\\" .. ADDON .. "\\Media\\Sounds\\RetailTimer\\"
local PATH_TEX = "Interface\\AddOns\\" .. ADDON .. "\\Media\\Textures\\RetailTimer\\"

--------------------------------------------------------------------------------
-- Start timer (BG / arena / Wintergrasp)
--------------------------------------------------------------------------------

local enableSound = true -- sound for big numbers countdown and finish
...
local SOUNDKIT = {
    UI_COUNTDOWN_TIMER = PATH_SOUND .. "Countdown.ogg",
    UI_COUNTDOWN_FINISHED = PATH_SOUND .. "Finish.ogg",
}
...
TIMER_NUMBERS_SETS["BigGold"] = { texture = PATH_TEX .. "BigTimerNumbers",
...
function Timer:SetGoTexture(timer)
    if ( timer.type == TIMER_TYPE_PVP ) then
        ...
        if factionGroup and factionGroup ~= "Neutral" then
            timer.GoTexture:SetTexture(PATH_TEX .. factionGroup .. "-Logo")
            timer.GoTextureGlow:SetTexture(PATH_TEX .. factionGroup .. "Glow-Logo")
        end
    elseif timer.type == TIMER_TYPE_CHALLENGE_MODE then
        timer.GoTexture:SetTexture(PATH_TEX .. "Challenges-Logo")
        timer.GoTextureGlow:SetTexture(PATH_TEX .. "ChallengesGlow-Logo")
...
frame2:SetStatusBarTexture(PATH_TEX .. "Raid-Bar-Hp-Fill")
...
border:SetVertexColor(K.Color.r, K.Color.g, K.Color.b, 1)
...
local Update3 = 1 -- Update is outside so it is not overwritten by OnUpdate while running
...
function frame:OnUpdate(elapsed)
    ...
    timeANDduration = time + Duration -- time is GetTime at countdown start, Duration is seconds to count
    ...
    if GetTime() >= timeANDduration - ( TimeForStartBigNumbers + 1 ) then
        frame4:OnUpdate() -- fade-out animation

    end

    if GetTime() >= timeANDduration - TimeForStartBigNumbers then
        enabled = true
        Timer:CreateTimer(1, exp - GetTime(), dur) -- start animated numbers

        frame:Hide() -- countdown over, disable OnUpdate

    end
end

function frame.Format_Text(seconds) -- format seconds as minutes:seconds
...
function frame.MSG_ArenaStart(gettime) -- take GetTime as arg and store it
    time = gettime
end

-- Status bar fade 0% -> 100% alpha
function frame3.OnUpdate(Frame)
    ...
    Frame:SetScript("OnUpdate", function(self, elapsed)
        ...
        frame:SetAlpha(Update2 / 1) -- bigger number = longer animation
...
function frame4.OnUpdate(Frame)
    ...
    Frame:SetScript("OnUpdate", function(self, elapsed)
        ...
        if Update3 <= 0 then
            Update3 = 1 -- reset so the next animation starts from one
            self:SetScript("OnUpdate", nil)
        end
    end)
end
...
Timer.CreateTimerBar = function(self, timerType, timeSeconds, totalTime)
    ...
    Duration = timeSeconds - 1 -- so big numbers start at 10 sec
...
function Timer:OnEvent(event, ...)
    ...
    elseif event == "PLAYER_ENTERING_WORLD" then
        enabled = true
        Update2 = 0 -- Update is inside the event so fade-in plays only on first arena entry, not on every msg
        frame:Hide() -- hide countdown when outside arena
        --dur = 0
        exp = 0 -- hide active countdown when leaving the battle
    end
end
...
--------------------------------------------------------------------------------
-- Invite timer (arena / battleground queue popup)
--------------------------------------------------------------------------------

local enableInviteSound = true -- sound on invite

local inviteFrame = CreateFrame("Frame", nil, StaticPopup1)

local inviteText = inviteFrame:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
inviteText:SetPoint("TOP", StaticPopup1, "BOTTOM", 0, 2)
inviteText:SetTextHeight(11)

local inviteText2 = inviteFrame:CreateFontString(nil, "OVERLAY", "GameFontNormal")
inviteText2:SetPoint("TOP", StaticPopup1, "TOP", 0, 20.5)
inviteText2:SetTextHeight(21)
inviteText2:SetShadowOffset(1.5, -1.5)

local inviteBar = CreateFrame("StatusBar", nil, inviteFrame)
inviteBar:SetFrameStrata("HIGH")
inviteBar:SetWidth(400)
inviteBar:SetHeight(12)
inviteBar:SetPoint("TOP", StaticPopup1, "BOTTOM", 0, 2)
inviteBar:SetOrientation("HORIZONTAL")
inviteBar:SetStatusBarTexture(PATH_TEX .. "Raid-Bar-Hp-Fill")
inviteBar:SetStatusBarColor(1, 0, 0, 1)
inviteBar:Hide()

local inviteBackground = inviteBar:CreateTexture(nil, "BACKGROUND")
inviteBackground:SetPoint("TOPRIGHT", inviteBar, "TOPRIGHT", 0, 0)
inviteBackground:SetPoint("BOTTOMLEFT", inviteBar, "BOTTOMLEFT", 0, 0)
inviteBackground:SetTexture("Interface\\Tooltips\\UI-Tooltip-Background")
inviteBackground:SetVertexColor(0, 0, 0, 0.5)

local inviteBackdrop = {
...
local inviteBorder = CreateFrame("Frame", nil, inviteBar)
inviteBorder:SetPoint("TOPRIGHT", inviteBar, "TOPRIGHT", 2, 2)
inviteBorder:SetPoint("BOTTOMLEFT", inviteBar, "BOTTOMLEFT", -2, -1.5)
inviteBorder:SetBackdrop(inviteBackdrop)
inviteBorder:SetBackdropColor(0, 0, 0, 0.0)
inviteBorder:SetBackdropBorderColor(0.5, 0.5, 0.5, 1)

local BattlefieldTime
local BattlefieldIndex = 1
local inviteEnabled
    ...
    if event == "UPDATE_BATTLEFIELD_STATUS" then
        for index = 1, MAX_BATTLEFIELD_QUEUES do
            ...
            if status == "confirm" then
                if enableInviteSound then
                    PlaySoundFile(PATH_SOUND .. "Countdown.ogg", "Master")
                end
                BattlefieldIndex = index
                BattlefieldTime = GetBattlefieldPortExpiration(index)
                if inviteEnabled then
                    inviteEnabled = false
                    inviteBar:SetMinMaxValues(0, GetBattlefieldPortExpiration(index))
                end
                inviteText2:SetText(name)
                inviteFrame:Show()
                inviteBar:Show()
                inviteBar:OnUpdate()
            end
...
inviteFrame:RegisterEvent("UPDATE_BATTLEFIELD_STATUS")
inviteFrame:RegisterEvent("PLAYER_ENTERING_WORLD")
inviteFrame:SetScript("OnEvent", InviteTimer_OnEvent)

local inviteHideFrame = CreateFrame("Frame")

function inviteBar.OnUpdate(Bar)
    Bar:SetScript("OnUpdate", function(self, elapsed)

        inviteText:SetText(string.format("0:%.f", GetBattlefieldPortExpiration(BattlefieldIndex)))

        BattlefieldTime = BattlefieldTime - elapsed

        self:SetValue(BattlefieldTime)

        if GetBattlefieldPortExpiration(BattlefieldIndex) <= 0 then
            inviteHideFrame:OnUpdate()
        end
    end)
end

-- Extra second of grace before hiding
function inviteHideFrame.OnUpdate(Frame)
...
StaticPopupDialogs["CONFIRM_BATTLEFIELD_ENTRY"].hideOnEscape = false -- disable hiding the popup with ESC
