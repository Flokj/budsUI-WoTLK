...
function f:gen_icon(unit, cond, party)
   ...
   if cond then
      ...
      if party then
         icon:SetTexture([[Interface\AddOns\]] .. K.Directory .. [[\Media\Worldmap\party]])
      end
   else
      u.elapsed = nil
...
function f:OnUpdate(icon, unit)
   ...
   if string.find(unit, "raid", 1, true) then
      ...
      icon:SetTexture(string.format([[Interface\AddOns\]] .. K.Directory .. [[\Media\Worldmap\Group%d]], group))
   end
