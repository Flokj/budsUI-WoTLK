...
Purpose:
	...
	Ported from KkthnxUI retail to WoW 3.3.5: power type 0 is mana here too,
	so the mana-only mode is unchanged apart from the classic PostUpdate
	signature (element, unit, cur, max).
-----------------------------------------------------------------------------]]
...
-- Anchor the raid health bar: it fills from the top down to the power bar
-- when power is shown, or to the frame bottom when it is not, so a hidden
-- power bar never leaves a dead gap.
...
-- Mana-only mode: show the power bar for mana users and let health fill for
-- the rest. Fires from oUF's power PostUpdate, so it tracks form and unit
-- changes. Power type 0 is mana.
local function ManaPowerPostUpdate(power, unit, cur, max)
	local self = power.__owner
	if not self then
		return
	end
	local ptype = unit and UnitPowerType(unit)
	local isMana = ptype ~= nil and ptype == 0
	...
	if power.__kkuiText then
		Module.UpdatePowerText(power, unit, cur, max)
	end
end
...
Module.Styles.Party = function(self)
	...
	-- with the member's level on the gradient strip above it.
...
Module.Styles.Raid = function(self)
	...
	-- Health is anchor-driven (no fixed height) so it can fill down to the
	-- power bar, or the whole frame when there is no power bar to show.
	Build.Health(self)
